    ...
    render(crises) {
        ...
        displayCrises.forEach((crisis, index) => {
            ...
            row.innerHTML = `
                            ...
                            <div id="btn-container-${crisis._id}">
                                ${crisis.dispatchStatus === 'PENDING_DRIVER' 
                                    ...
                                    : (crisis.dispatchStatus === 'IN_TRANSIT' || crisis.status === 'MONITORING')
                                        ? `<button disabled class="opacity-50 cursor-not-allowed text-[9px] font-bold tracking-widest uppercase text-gray-400 border border-gray-500/30 bg-gray-500/10 rounded-full px-3 py-1 whitespace-nowrap overflow-hidden text-ellipsis max-w-full">En Route ${crisis.assignedDriverId ? `(${fleetManager.vehicles.get(crisis.assignedDriverId)?.name || 'UNIT ' + crisis.assignedDriverId.substring(0, 4)})` : ''}</button>`
                ...
                <!-- Requests Accordion (Hidden initially) -->
                <div id="requests-container-${crisis._id}" class="hidden w-full mt-4 bg-lumenaDark/5 dark:bg-lumenaLight/5 rounded-lg p-4">
                    <div class="text-center opacity-40 text-xs font-bold py-2">Loading...</div>
                </div>
            `;
